...
func reorderList(head *ListNode) {
	...
	for right != nil { // правая часть всегда будет равна либо короче левой
		// временные указатели на следующие узлы в левом и правом списке
		tmpLeftNext, tmpRightNext := left.Next, right.Next
		
		// следующим в левом списке делаем указатель на правый список
		left.Next = right
		// следующим в правом списке делаем тот узел, что был следующим в левом списке
		right.Next = tmpLeftNext 

		// сдвигаем указатели, в левый указатель берем тот узел, что и был в левом списке
		// в правый указатель берем тот узел, который следующий в правом списке
		left, right = tmpLeftNext, tmpRightNext 
	}
